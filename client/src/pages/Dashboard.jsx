...
const Dashboard = ({ content, rows, apiData, filtersBasedOn }) => {
  ...
  const groupsBy = (group, data) => {
    if (group && data) {
      data.map((ele, index) => {
        return (ele['yoy'] = generateRandomDate(new Date('2023-04-01'), new Date('2023-08-01'))[
          index
        ]);
      });
  ...
  const getContent = (template, type, data) => {
    ...
    if (type === 'table') {
      ...
      if (template?.quadrantDataKey && template?.customRowConfig) {
        ...
        if (
          tableData.data &&
          tableData.data[tableData.data.length - 1].channel !== template?.customRowConfig?.key
        ) {
  ...
  return (
    <>
      <div className={`grid  gap-2 ${generateClasses(content?.filterData?.parent?.style)}`}>
        <FilterComponent
          filters={content.filterData}
          style={generateClasses(content.filterData.style)}
          key={`_id_${currentTab}_`}
        />
      </div>

      {rows.map((row) => (
        <div className="grid grid-cols-5 gap-3 mt-5 mx-4" key={row.id}>
          {row.dashboardContent.quadrants &&
            row.dashboardContent.quadrants.map((quadrant) => (
              <div
                className={`${generateClasses(quadrant?.style)} ${generateClasses(
                  quadrant,
                )} relative`}
                key={quadrant?.id}
              >
